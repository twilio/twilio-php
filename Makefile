...
# phpDocumentor runs on PHP 8.x. The phar is committed to the repo, as phpdox.phar
# is -- phpdoc.org is not reachable from the CI build network.
docs:
	php ${PHPDOC_PHAR} -d src/ -t docs/api

# Previous generator. Needs PHP 7.4, which the CI image does not have -- the PHP74
# fallback silently runs it under 8.x. Kept until the phpDocumentor output is signed off.
docs-phpdox:
	${PHP74} ${PHPDOX_PHAR}
