...
class WorkflowsTest extends PHPUnit_Framework_TestCase
{
    function testCreate()
    {
        ...
        $http->shouldReceive('post')->once()
            ->with('/v1/Accounts/AC123/Workspaces/WS123/Workflows.json',
        ...
        $workflow = $wdsClient->workspace->workflows->create('Test Workflow', 'configuration', 'http://www.example.com');
    ...
    function testGet()
    {
        ...
        $http->shouldReceive('get')->once()
            ->with('/v1/Accounts/AC123/Workspaces/WS123/Workflows/WF123.json')
            ->andReturn(array(200, array('Content-Type' => 'application/json'),
                json_encode(array('sid' => 'WF123', 'friendly_name' => 'Test Workflow'))
            ));
        $wdsClient = new Wds_Services_Twilio('AC123', '123', 'WS123', 'v1', $http);
        $workflow = $wdsClient->workspace->workflows->get('WF123');
        $this->assertNotNull($workflow);
