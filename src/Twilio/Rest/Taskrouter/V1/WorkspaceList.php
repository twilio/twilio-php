...
class WorkspaceList extends ListResource
    ...
    {
        $response = $this->_create( $friendlyName, $options);
        $resource = new WorkspaceInstance(
                        $this->version,
                        $response->getContent()
                    );
    ...
    /**
     * Streams WorkspaceInstance records from the API as a generator stream and returns result with Metadata
     * This operation lazily loads records as efficiently as possible until the
     * limit
     * is reached.
     * The results are returned as a generator, so this operation is memory
    ...
    /**
     * Retrieve a single page of WorkspaceInstance records with metadata
     * Request is executed immediately
     *
     * @param mixed $pageSize Number of records to return, defaults to 50
     * @param string $pageToken PageToken provided by the API
     * @param mixed $pageNumber Page Number, this value is simply for client state
     * @return PageMetadata of WorkspaceInstance
     */
    ...
    ): PageMetadata
    {
        ...
        $resource =  new WorkspacePage($this->version, $response, $this->solution);
