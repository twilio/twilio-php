...
class EntityList extends ListResource
     ...
     */
    private function _create(string $identity): Response
    {
        $data = Values::of([
            'Identity' =>
                $identity,
    ...
    {
        $response = $this->_create( $identity);
    ...
    /**
     * Create the EntityInstance with Metadata
     *
     * @param string $identity The unique external identifier for the Entity of the Service. This identifier should be immutable, not PII, length between 8 and 64 characters, and generated by your external system, such as your user's UUID, GUID, or SID. It can only contain dash (-) separated alphanumeric characters.
     * @return ResourceMetadata The Created Resource with Metadata
     * @throws TwilioException When an HTTP error occurs.
     */
    public function createWithMetadata(string $identity): ResourceMetadata
    {
        $response = $this->_create( $identity);
        $resource = new EntityInstance(
                        $this->version,
                        $response->getContent(),
                        $this->solution['serviceSid']
                    );
    ...
    /**
     * Streams EntityInstance records from the API as a generator stream and returns result with Metadata
     * This operation lazily loads records as efficiently as possible until the
     * limit
     * is reached.
     * The results are returned as a generator, so this operation is memory
    ...
    /**
     * Retrieve a single page of EntityInstance records with metadata
     * Request is executed immediately
     *
     * @param mixed $pageSize Number of records to return, defaults to 50
     * @param string $pageToken PageToken provided by the API
     * @param mixed $pageNumber Page Number, this value is simply for client state
     * @return PageMetadata of EntityInstance
     */
    ...
    ): PageMetadata
    {
        ...
        $resource =  new EntityPage($this->version, $response, $this->solution);
